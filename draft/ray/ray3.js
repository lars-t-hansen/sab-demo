...
// With a 256-element cache:
//
// In FF33 this takes approx 10s vs approx 9s for ray2.js - pretty good.
// In Chrome this takes approx 11s vs approx 9s for ray2.js.
//
//
// Without the 256-element cache:
//
// - Chrome increases from 11s to 35s
// - Firefox increases from 10s to 20s
//
// Caches smaller than the object population don't make much sense,
// which is not surprising, given that the ray tracer examines all
// during every run.  (So it's not a cache but a lazily constructed
// local copy of the scene graph.)  The object population is about
// 170.
...
function read_ObjectAt(i) {
    if (om[i] == TRIANGLE)
	return read_Triangle(i);
    else
	return read_Sphere(i);
}
