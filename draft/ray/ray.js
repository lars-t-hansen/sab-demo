...
const debug = true;		// Progress printout, may confuse the consumer
...
const m1 = new Array(4096);
const m2 = new Array(4096);
...
function new_Material(diffuse, specular, shininess, ambient, mirror) {
...
function scene_intersect(eye, ray, min, max) {
    var min_obj = 0;
    var min_dist = SENTINEL;
    
    // Open-coding the intersect call makes very little difference (FF33).

    for ( var obj_=3, limit=om.length ; obj_ < limit ; obj_+=3 ) {
	if (om[obj_])
	    var tmp = sphere_intersect(obj_, eye, ray, min, max);
	else
	    var tmp = triangle_intersect(obj_, eye, ray, min, max);
	var obj = tmp.obj;
	var dist = tmp.dist;
	if (obj)
...
function new_Sphere(material, center, radius) {
    var p = falloc(center.x, center.y, center.z, radius);
    oalloc(SPHERE, material, p);
}

function sphere_intersect(obj_, eye, ray, min, max) {
    const data_ = om[obj_+2];
    const center = read3(data_);
    const radius = read(data_+3); 
    var DdotD = dot(ray, ray);
    var EminusC = sub(eye, center);
    var B = dot(ray, EminusC);
    var disc = B*B - DdotD*(dot(EminusC,EminusC) - radius*radius);
    if (disc < 0.0)
	return {obj:0, dist:0};
    ...
    if (_dist == SENTINEL)
	return {obj:0, dist:0};
    return {obj:obj_, dist:_dist};
}

function sphere_normal(obj_, p) {
    const data_ = om[obj_+2];
    const center = read3(data_);
    const radius = read(data_+3); 
    return divi(sub(p, center), radius);
}

function new_Triangle(material, v1, v2, v3) {
    var p = falloc(v1.x, v1.y, v1.z, v2.x, v2.y, v2.z, v3.x, v3.y, v3.z);
    oalloc(TRIANGLE, material, p);
}


function triangle_intersect(obj_, eye, ray, min, max) {
    const data_ = om[obj_+2];
    const v1 = read3(data_);
    const v2 = read3(data_+3);
    const v3 = read3(data_+6);
    var a = v1.x - v2.x;
    var b = v1.y - v2.y;
    var c = v1.z - v2.z;
    var d = v1.x - v3.x;
	...
	return {obj:0,dist:0};
    return {obj:obj_, dist:t};
}

function triangle_normal(obj_, p) {
    const data_ = om[obj_+2];
    const v1 = read3(data_);
    const v2 = read3(data_+3);
    const v3 = read3(data_+6);
...
const intersect = [triangle_intersect, sphere_intersect];
const normal = [triangle_normal, sphere_normal];
...
function setStage() {
	...
	new_Triangle(m, v1, v2, v3);
	new_Triangle(m, v1, v3, v4);
    }
    ...
    const m1 = new_Material(DL3(0.1, 0.2, 0.2), DL3(0.3, 0.6, 0.6), 10, DL3(0.05, 0.1, 0.1), 0);
    const m2 = new_Material(DL3(0.3, 0.3, 0.2), DL3(0.6, 0.6, 0.4), 10, DL3(0.1,0.1,0.05),   0);
    const m3 = new_Material(DL3(0.1,  0,  0), DL3(0.8,0,0),     10, DL3(0.1,0,0),     0);
    const m4 = new_Material(muli(darkGray,0.4), muli(darkGray,0.3), 100, muli(darkGray,0.3), 0.5);
    const m5 = new_Material(muli(paleGreen,0.4), muli(paleGreen,0.4), 10, muli(paleGreen,0.2), 1.0);
    const m6 = new_Material(muli(yellow,0.6), zzz, 0, muli(yellow,0.4), 0);
    const m7 = new_Material(muli(red,0.6), zzz, 0, muli(red,0.4), 0);
    const m8 = new_Material(muli(blue,0.6), zzz, 0, muli(blue,0.4), 0);

    new_Sphere(m1, DL3(-1, 1, -9), 1);
    new_Sphere(m2, DL3(1.5, 1, 0), 0.75);
    new_Triangle(m1, DL3(-1,0,0.75), DL3(-0.75,0,0), DL3(-0.75,1.5,0));
    new_Triangle(m3, DL3(-2,0,0), DL3(-0.5,0,0), DL3(-0.5,2,0));
    ...
    for ( var i=0 ; i < 30 ; i++ )
	new_Sphere(m6, DL3((-0.6+(i*0.2)), (0.075+(i*0.05)), (1.5-(i*Math.cos(i/30.0)*0.5))), 0.075);
    for ( var i=0 ; i < 60 ; i++ )
	new_Sphere(m7, DL3((1+0.3*Math.sin(i*(3.14/16))), (0.075+(i*0.025)), (1+0.3*Math.cos(i*(3.14/16)))), 0.025);
    for ( var i=0 ; i < 60 ; i++ )
	new_Sphere(m8, DL3((1+0.3*Math.sin(i*(3.14/16))), (0.075+((i+8)*0.025)), (1+0.3*Math.cos(i*(3.14/16)))), 0.025);
...
function raycolor(eye, ray, t0, t1, depth) {
    var tmp = scene_intersect(eye, ray, t0, t1);
    var obj = tmp.obj;
    var dist = tmp.dist;

    if (obj) {
	const m = readM(om[obj+1]);
	const p = add(eye, muli(ray, dist));
	const n1 = normal[om[obj]](obj, p);
	const l1 = normalize(sub(light, p));
	var c = m.ambient;
	var min_obj = null;

	if (shadows) {
	    var tmp = scene_intersect(add(p, muli(l1, EPS)), l1, EPS, SENTINEL);
	    min_obj = tmp.obj;
	}
